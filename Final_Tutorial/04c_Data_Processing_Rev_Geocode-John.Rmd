...
# Converting coordinates to zip codes

There was difficulty with initally processing the location data into a format for analysis. For the crime and census data:
...
## Sources for latitude and longitude to zipcode code and datta
...
First the crime data is read in:

```{r eval=FALSE}
sf <- sf::st_read("nyc_zip_code_tabulation_areas_polygons.geojson")
```

Then the desired latitude and longitude fields are taken. the complaint number is also taken so that the crime-zipcode datat can be joined again later. The NA values are omitted.
...
The coordinates are processed with the st_witthin function and put into an output using sapply:
...
The most important element was the corresponding populations for each zipcode. The population data for each demographic was summed accross each zipcode. The demogrpahic information was initially stored as percents, so it was necessary to multiply those percents by the total population for each census tract.
...
Finally, the columns were renamed as needed. The data for zipcode to population demographic was saved for analylsis later:
...
## Source for census tract to zipccode data

The folllowing link includes thte updated census tract to zipcode information: https://www.huduser.gov/portal/datasets/usps_crosswalk.html
...
Server is the part of shiny application where processing and updating of data, map, predicted values happen. We'll gradually build the code for Sever in steps. The inputId in ui.R script will be linked in Sever script.
...
We'll using Neural net and Random forest models to compute prices of real estate based on 4 inputs from user
```
load("neural_net.rda")
...
```
## Adding color palette, mapping the reactive data based on the attribute chosen by the user on radioButton and time chosen on sliderInput

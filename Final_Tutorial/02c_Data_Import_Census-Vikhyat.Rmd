...
The United States census bureau data estimates for 2015 were downloaded for all 2,167 individual NYC census tracts. This data was in two parts. The first part which included the block code/census tract, its latitude and longitude as well as the name of the county and state was imported throug the following code. After getting them in a dataframe (block_loc), the rows with 'NA' were removed. A basic 'unique value check' showed us that there were 38,936 locations in 20,406 blocks in 15 counties. These longitude and latitude locations would help us later when we classified the demographic data by zip code.
...
```
<br/>
We divided this dataframe into different counties in NY and NJ to get a better sense about the data. This gave us information about the number of locations in each of the counties in our data. 
...
The second part of the census data was the NYC census tracts data. This was also split into different dataframes which contained data about sex, race, income, jobs, commute and employment, albeit these dataframes were not used separately for analysis in the future. 
```{r eval = FALSE}
